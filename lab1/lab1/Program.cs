...
namespace lab1
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            int resolution = 216; //6 //36 //216 //648 //1296 //3888 //7776
            int recDepth = 3;
            ...
            Thread thread = new Thread(() =>
            {
                ...
                using (FileStream file = new FileStream("test2.bmp", FileMode.Create, FileAccess.Write))
                {
                    Array.Copy(BitConverter.GetBytes((int)resolution), 0, header, 0x12, sizeof(int));
                    Array.Copy(BitConverter.GetBytes((int)resolution), 0, header, 0x16, sizeof(int));
        ...
        static void fillWhite(byte[,] table)
        {
            fillWhiteRecursive(table, 0, 0, table.GetLength(0), table.GetLength(1));
        }

        static void fillWhiteRecursive(byte[,] table, int x, int y, int width, int height)
        {
            if (y >= height) return;
            ...
            {
                table[y, x] = 1;
                fillWhiteRecursive(table, x + 1, y, width, height);
            }
            else
            {
                fillWhiteRecursive(table, 0, y + 1, width, height);
            }
        }
        ...
        static void drawColorSquare(int x, int y, int width, byte[,] table, byte color)
        {
            ...
            int maxWidth = table.GetLength(1);

            if (x >= maxWidth || y >= height) return;
            if (width == 0) return;

            if (x < width)
            {
                table[y, x] = color;
                drawColorSquare(x + 1, y, width, table, color);
            }
            else
            {
                drawColorSquare(0, y + 1, width, table, color);
            }
        }
        ...
        static void drawFinishGrid(int x, int y, int width, byte[,] table)
        {
            drawVerticalRecursive(x + width / 3, y, width, table, 1);
            drawHorizontalRecursive(x, y + width / 3, width, table, 1);
        }
        ...
        {
            int w6 = width / 6;
            drawVerticalRecursive(x + w6, y, width, table, 1);
            drawHorizontalRecursive(x, y + w6, width, table, 1);
        }

        static void drawVerticalRecursive(int x, int y, int width, byte[,] table, int i)
        {
            if (i >= 6) return;
            ...
            drawVerticalRecursive(x + width / 6, y, width, table, i + 1);
        }

        static void drawHorizontalRecursive(int x, int y, int width, byte[,] table, int i)
        {
            if (i >= 6) return;
            ...
            drawHorizontalRecursive(x, y + width / 6, width, table, i + 1);
        }
        ...
        static byte[] createBmp(byte[,] table)
        {
            ...
            createBmpRecursive(table, bpm, 0, 0, width, line);

            return bpm;
        }

        static void createBmpRecursive(byte[,] table, byte[] bpm, int y, int x, int width, long line)
        {
            if (y >= width) return;
            ...
            if (x < width)
            {
                ...
                ushort color = (pixel == 1) ? (ushort)0xFFFF : (ushort)0x0000;
                long pos = y * line + x * 2;
                bpm[pos] = (byte)(color & 0xFF);
                bpm[pos + 1] = (byte)((color >> 8) & 0xFF);

                createBmpRecursive(table, bpm, y, x + 1, width, line);
            }
            else
            {
                createBmpRecursive(table, bpm, y + 1, 0, width, line);
            }
        }
    }
}
